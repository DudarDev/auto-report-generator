...
load_dotenv()
# ... (решта коду gsheet.py, як у попередній версії Canvas, 
#      переконайтеся, що _normalize_data використовує APP_INTERNAL_KEYS) ...
# Ось повний код для gsheet.py з попереднього разу, з виправленим імпортом:
...
def get_sheet_data(sheet_id: Optional[str] = None, 
    ...
    # APP_INTERNAL_KEYS тепер імпортовано з app.config_fields
    if sheet_id:
        try:
